...
%   $Revision: 1.18.4.9 $ $Date: 2011/04/12 20:42:08 $

% if signal is dropped use zeros else pb index will spike
SIG(SIG<0)=0;
...
for k = 1:nb_SCALES
    ...
    f            = fliplr(val_WAV(j));
    coefs(ind,:) = -1*wkeep1(diff(wconv1(ySIG,f)),lenSIG);
    ind          = ind+1;
end
...
[NBC,err1,err2,ytics,ylabs,titleSTR] = deal(plotPARAMS{1:6});
