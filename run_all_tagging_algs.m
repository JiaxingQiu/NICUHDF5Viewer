        ...
        if exist('tagcol')
            if isfirst
                ...
                if ~isfile && ~isempty(tagcol) % If there is no results file and we have something to put in one, create a file!
                    [result_name,result_data,result_tags,result_tagcolumns,result_tagtitle,result_qrs] = createresultsfile(resultname(algnum-3,:),result,t_temp,tag,tagcol,[]);
                ...
                elseif isfile && ~isempty(tagcol) % If there is a results file and we have something to put in it, put it in!
                    [result_name,result_data,result_tags,result_tagcolumns,result_tagtitle,result_qrs] = addtoresultsfile3(resultname(algnum-3,:),result,t_temp,tag,tagcol,[],result_name,result_data,result_tags,result_tagcolumns,result_tagtitle,result_qrs);
